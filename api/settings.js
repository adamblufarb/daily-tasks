...
module.exports = withAuth(async (req, res, userId) => {
  ...
  const hska = has_shown_key_affix_tutorial  !== undefined ? has_shown_key_affix_tutorial  : (current?.has_shown_key_affix_tutorial ?? false);
  ...
  await sql`
    INSERT INTO settings_v2
      (user_id, reset_hour, has_completed_onboarding, onboarding_ritual_count,
       has_shown_record_tutorial, has_shown_bazaar_tutorial, has_shown_actpool_tutorial,
       has_shown_key_affix_tutorial)
    VALUES (${userId}, ${rh}, ${hco}, ${orc}, ${hsr}, ${hsb}, ${hsap}, ${hska})
    ON CONFLICT (user_id) DO UPDATE SET
      ...
      has_shown_key_affix_tutorial = ${hska}
  `;
